...
<body class="bg">
	<?php
		...
		// This is for Modal
		include 'Modal/refresh_training_modal.php';
	?>
	<div class="card_opa">
		<br>
		<div class="row">
			<div class="md-form mb-0 col-sm-3">
				<input type="text" id="Search" class="form-control text-center ml-3" oninput="search_refresh()">
		...
		<div class="col-sm-12 d-flex justify-content-center">
			<p class="h3 mt-2" id="for_or_ongoing_title" >Incomplete in Refresh Training</p>
		</div>
		<div class="col-sm-12">
...
<script>
	search_all();// For Viewing All Incomplete in Refresh Training
	function search_all(){// For Viewing All Incomplete in Refresh Training
		...
		xhttp.open("GET", "AJAX/query_refresh.php?operation=search_all_incomplete", true);
		xhttp.send();
	}
	function search_refresh(){ // For Searching All Incomplete in Refresh Training
		...
		xhttp.open("GET", "AJAX/query_refresh.php?operation=search_keyword_incomplete&&keyword="+keyword, true);
		xhttp.send();
	}
	function open_details(x){
		...
		xhttp.onreadystatechange = function(){
			if (this.readyState == 4 && this.status == 200){
				...
				get_history_process(x);
				document.getElementById('id_no_cc').value=id_no;
				document.getElementById('title_modal_head').innerHTML="Completed in Refresh Training"; // To Change the Title on the Header of Modal
	...
	function get_history_process(x){
		var xhttp = new XMLHttpRequest();
		xhttp.onreadystatechange = function(){
			if (this.readyState == 4 && this.status == 200){
				var response = this.responseText;
				document.getElementById('table_content_history').innerHTML=response;
			}
		};
		xhttp.open("GET", "AJAX/get_history.php?operation=get_history_process&&id_no="+x, true);
		xhttp.send()
	}
	function btn_save_status_function(){
		var id_no = document.getElementById('id_no_cc').value;
		var process_dropdown = document.getElementById('process_dropdown').value;
		var pt_section_select = document.getElementById('pt_section_select').value;
		...
		xhttp.onreadystatechange = function(){
			if (this.readyState == 4 && this.status == 200){
				var response = this.responseText;
				document.getElementById('output_ajax').value=response;
			}
		};
		xhttp.open("GET", "AJAX/save_refresh.php?operation=save&&id_no="+id_no+"&&process_dropdown="+process_dropdown+"&&pt_section_select="+pt_section_select+"&&process_under="+process_under+"&&date_of_training="+date_of_training+"&&name_trainor="+name_trainor, true);
		xhttp.send()
	}
	function download_excel(){
		window.open("AJAX/download_excel/incomplete_refresh_training.php" , '_blank');
	}
</script>
<!-- JQuery -->
