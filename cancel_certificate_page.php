...
<head>
	...
	<title>Cancellation of Certificate or Refresh </title>
	<!-- Title Logo -->
...
<body class="bg">
	<?php
		...
		include 'Modal/view_details_cc.php';
	?>	
	<div class="card_opa">
		<br>
		<div class="row">
			<div class="md-form mb-0 col-sm-3">
				<input type="text" id="Search" class="form-control text-center ml-3" oninput="search_cc()">
		...
		<div class="col-sm-12 d-flex justify-content-center">
			<p class="h3 mt-2">Cancelled Certificate</p>
		</div>
		<div class="col-sm-12">
			<table class="table table-bordered">
				<thead class="blue-grey lighten-4">
					<tr>
						...
						<th><strong>Violation in Specific  Process</strong></th>
						<th><strong>Total No. of Violation</strong></th>
					</tr>
				</thead>
				<tbody id="table_content">
					<tr>
						<td colspan="10"><div class="loader_default">Loading...</div></td>
					</tr>
				</tbody>
			</table>
			<br>
			<!--div class="rounded-circle default-color card-img-100 mx-auto mb-1 pulse" style="margin-top:-10px;width:50px;height:50px;cursor:pointer;">
				<i class="fas fa-redo text-white" style="margin-left:17px;margin-top:16px;"></i>
			</div-->
			<br>
		</div>
	</div>
<script>
	...
	function search_all(){
		...
		xhttp.open("GET", "AJAX/query_cc_search.php?operation=search_all", true);
		xhttp.send();
	}
	function search_cc(){
		...
		xhttp.open("GET", "AJAX/query_cc_search.php?operation=search_cc&&keyword="+keyword, true);
		xhttp.send();
	}
	function open_details_cc(x){
		var split = x.split('~!~');
		var id_no = split[0];
		var process_final_initial = split[1];
		var practice_training = split[2];
		var process_under = split[3];
		load_practice();
		load_certification(id_no,process_final_initial,practice_training,process_under);
		document.getElementById('id_no_cc').value=id_no;
		document.getElementById('process_final_initial_hidden').value=process_final_initial;
		document.getElementById('practice_training_hidden').value=practice_training;
		document.getElementById('process_under_hidden').value=process_under;
		$("#View_Details_CC").modal();
		var xhttp = new XMLHttpRequest();
		xhttp.onreadystatechange = function(){
			if (this.readyState == 4 && this.status == 200){
				var response = this.responseText;
				document.getElementById('table_content_cc').innerHTML=response;
			}
		};
		xhttp.open("GET", "AJAX/query_cc_search.php?operation=search_all_by_id&&id_no="+id_no, true);
		xhttp.send();
	}
	function load_certification(a,b,c,d){
		var xhttp = new XMLHttpRequest();
		xhttp.onreadystatechange = function(){
			if (this.readyState == 4 && this.status == 200){
				var response = this.responseText;
				if(response !='' ){
					...
					var certificate = split[0];
					var date_issued = split[1];
					var status = split[2];
					document.getElementById('certificate').value=certificate;
					document.getElementById('date_issued').value=date_issued;
					document.getElementById('cc_or_refresh').value=status;						
				}
			}
		};
		xhttp.open("GET", "AJAX/query_cc_refresh.php?operation=certification&&id_no="+a+"&&process_final_initial="+b+"&&practice_training="+c+"&&process_under="+d, true);
		xhttp.send();
	}
	function open_on_large(x){
		...
		xhttp.onreadystatechange = function(){
			if (this.readyState == 4 && this.status == 200){
				...
				document.getElementById('process_under').value=process_under;
				...
				load_car_model1(car_model,line);
			}
		};
	...
	function display_hidden_details(){
		...
		document.getElementById('department_section').style.display="inline-block";
		...
		document.getElementById('car_model_section').style.display="inline-block";
		document.getElementById('car_model_line_section').style.display="inline-block";
	}
	function display_none_details(){
		...
		document.getElementById('car_model_section').style.display="none";
		document.getElementById('car_model_line_section').style.display="none";
	}
	function close_modal(){
		$('#View_Details_CC').modal('toggle');
	...
	function btn_save_status_function(){
		var id_no_cc = document.getElementById('id_no_cc').value;
		var cc_or_refresh = document.getElementById('cc_or_refresh').value;
		var certificate = document.getElementById('certificate').value;
		var date_issued = document.getElementById('date_issued').value;
		var process_final_initial = document.getElementById('process_final_initial_hidden').value;
		var practice_training = document.getElementById('practice_training_hidden').value;
		var process_under = document.getElementById('process_under_hidden').value;
		var xhttp = new XMLHttpRequest();
		xhttp.onreadystatechange = function(){
			if (this.readyState == 4 && this.status == 200){
				var response = this.responseText;
		...
		xhttp.open("GET", "AJAX/query_cc_refresh.php?operation=save&&id_no_cc="+id_no_cc+"&&cc_or_refresh="+cc_or_refresh+"&&certificate="+certificate+"&&date_issued="+date_issued+"&&process_final_initial="+process_final_initial+"&&practice_training="+practice_training+"&&process_under="+process_under, true);
		xhttp.send();
	}
	function show_less(){
	...
	function download_excel(){
		window.open("AJAX/download_excel/cancelled_certificate.php" , '_blank');
	}
</script>
<!-- JQuery -->
