...
<body class="bg">
	...
	<div class="card_opa">
		...
		<div class="col-sm-12 d-flex justify-content-center">
			<p class="h3 mt-2">Passed in Practice Training</p>
		</div>
		<div class="col-sm-12">
...
<script>
	...
	function search_all(){
		...
		xhttp.open("GET", "AJAX/query_practice.php?operation=search_all_passed", true);
		xhttp.send();
	}
	function search_practice(){
		...
		xhttp.open("GET", "AJAX/query_practice.php?operation=search_key_passed&&keyword="+keyword, true);
		xhttp.send();
	}
	function open_details(x){
		...
		document.getElementById('pt_section_result').disabled = true;
		document.getElementById('trainor_section').style.display ="inline-block";
		document.getElementById('name_trainor').disabled = true;
		document.getElementById('btn_save_status').style.display="none";
		document.getElementById('process_dropdown').style.display="none";
		document.getElementById('title_modal_head').innerHTML="Passed in Practice Training"; // To Change the Title on the Header of Modal
	...
	function get_result_trainor(x){
		var split = x.split('~!~');
		var id_no = split[0];
		var id = split[1];
		document.getElementById('pt_section_result').value='';
		document.getElementById('name_trainor').value='';
		var xhttp = new XMLHttpRequest();
		xhttp.onreadystatechange = function(){
			if (this.readyState == 4 && this.status == 200){
				var response = this.responseText;
				var split = response.split('~!~');
				document.getElementById('pt_section_result').value = split[0];
				document.getElementById('name_trainor').value = split[1];
			}
		};
		xhttp.open("GET", "AJAX/get_result_trainor.php?operation=get_result_trainor&&id_no="+id_no+"&&id="+id, true);
		xhttp.send()
	}
	function download_excel(){
		window.open("AJAX/download_excel/passed_practice_training.php" , '_blank');
	}
</script>
<!-- JQuery -->
