...
if($operation == 'save'){
	...
	if ($result->num_rows > 0){
		while($row = $result->fetch_assoc()){
			...
			$sql1 = "INSERT INTO history_process (id_no, process_final_initial, practice_training, process_under)
			VALUES ('$id_no', '$process_final_initial', '$practice_training', '$process_under')";
			if ($conn->query($sql1) === TRUE) {
				echo "New record created successfully";
			} else {
				echo "Error: " . $sql1 . "<br>" . $conn->error;
			}
			$sql2 = "INSERT INTO refresh_training_details (id_no, process_final_initial, practice_training, process_under, trainor, date_training, result, remarks, status)
			...
			$sql3 = "DELETE FROM cc_or_refresh WHERE id_no='$id_no'";
	...
	$sql = "UPDATE refresh_training_details SET trainor='$name_trainor', date_completed='$date_completed', result='$pt_section_result', status='$pt_section_result' WHERE id='$id' AND id_no='$id_no'";
